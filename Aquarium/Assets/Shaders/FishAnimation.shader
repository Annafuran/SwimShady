Shader "Custom/FishAnimation"
{
    Properties{
        ...
        v2f vert(appdata_base v)
        {
            ...
            v.vertex.y += cos((v.vertex.z + _Time.y * _SpeedY)* _FrequencyX)* _AmplitudeX;

            //Movement in the X AXIS
            if (v.vertex.z > _HeadLimit)
            {
                v.vertex.x += sin((0.005 + _Time.y * _SpeedZ) * _FrequencyZ)* _AmplitudeZ * _HeadLimit;
            }
            else
            {
